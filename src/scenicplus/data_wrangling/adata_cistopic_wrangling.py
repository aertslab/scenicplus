...
def _generate_pseudocells_for_numpy(X: np.ndarray,
                                   grouper: Groupby,
                                   nr_cells: list,
                                   nr_pseudobulks: list,
                                   axis=0):
...
def _generate_pseudocell_names(grouper: Groupby,
                              nr_pseudobulks: list,
                              sep='_'):
    if len(nr_pseudobulks) != len(grouper.indices):
        raise ValueError(
...
def process_non_multiome_data(
    ...
    key_to_group_by: str,
    use_raw_for_GEX_anndata: bool = True,
    imputed_acc_kwargs: Dict[str, Any] = {'scale_factor': 10**6},
    ...
    if type(nr_metacells) is int:
    ...
    elif type(nr_metacells) is dict:
        # it is a mapping
        ...
        elif type(nr_cells_per_metacells) is dict:
            l_nr_metacells = []
            for k in grouper_EXP.keys:  # for this we need the assertion above
                nr_cells_wi_annotation = min(sum(GEX_anndata.obs[key_to_group_by].to_numpy() == k),
                                                sum(cisTopic_obj.cell_data[key_to_group_by].to_numpy() == k))
