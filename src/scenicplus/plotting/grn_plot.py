...
def layout_grn(G: Graph,
    ...
    for gene in gene_nodes:
        ...
        if all([r in regions_targetting_TFs for r in regions]):
            ...
            continue
        pos_regions_gene = np.array([pos_regions[r] for r in regions if r not in regions_targetting_TFs])
        if len(regions) > 1:
            ...
            furthest_points = np.unravel_index(pairwise_distances.argmax(), pairwise_distances.shape)
            m, b = _line_two_points(pos_regions_gene[furthest_points[0]], pos_regions_gene[furthest_points[1]], return_func = False)
            ...
            m, b = _line_two_points(pos_regions_gene[0], [0, 0], return_func=False)
    ...
    for TF in TF_nodes:
        ...
        if all([r in regions_targetting_TFs for r in regions]):
            ...
            continue
        pos_regions_TF = np.array([pos_regions[r] for r in regions if r not in regions_targetting_TFs])
        if len(regions) > 1:
            #get the positions which are furthest apart and "draw" a line through them
            pairwise_distances = _pairwise_distance(pos_regions_TF)
            furthest_points = np.unravel_index(pairwise_distances.argmax(), pairwise_distances.shape)
            m, b = _line_two_points(pos_regions_TF[furthest_points[0]], pos_regions_TF[furthest_points[1]], return_func = False)
            #draw a perpendicular line through the first line and the mean position
            ...
            #draw line trough origin and pos of region
            m, b = _line_two_points(pos_regions_TF[0], [0, 0], return_func=False)
            #get the point which is a distance dist_genes away from point pos_regions_gene 
    ...
    pos_regions_TF = {k: pos_regions_TF[k] for k in pos_regions_TF.keys() if k in regions_targetting_TFs}
...
def draw_TF_edges(G: Graph,
    ...
    if return_drawn_edges:
        edges_drawn = []
    for TF in list(set(TF_nodes) - set(TFs_to_highlight)) + TFs_to_highlight: #this set operation makes sure TFs_to_highlight are drawn on top
        layout = high_light_layout if TF in TFs_to_highlight else non_high_light_layout
        TF_target_regions_on_circle = set(source_target_dict[TF]) - set(regions_targetting_TFs)
        TF_target_regions_on_circle_pos = np.array([pos[r] for r in TF_target_regions_on_circle])
        if return_drawn_edges:
        ...
        for ind in grouper.indices:
            ...
            for target_point in region_pos:
                ...
                con = FancyArrowPatch(  path = path, arrowstyle =arrowstyle, mutation_scale = mutation_scale, **layout)
                ax.add_artist(con)
    if return_drawn_edges:
        return edges_drawn
...
def draw_grn(   G: Graph,
    ...
    layouts = {k: [node_layouts[node][k] for node in G.nodes] for k in (set(POSSIBLE_LAYOUTS) - set(['label']))}
    ...
    nx.draw_networkx_labels(G, pos, labels=labels, font_size=20, font_family='sans-serif', font_color=label_font_color, font_weight = 'bold', ax = ax)
